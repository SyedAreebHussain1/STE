...
import { data } from "./templates/templateData";
...
const PitchDeckSlides = (props: Props) => {
  ...
  return (
    <div>
      ...
      <div className="flex justify-end items-center my-4 w-full gap-3">
        <RoundedButton title={"Download"} type="primary" sm bold   onClick={handlePrint}  />
        <RoundedButton
          title={"Edit Pitch Deck"}
          type="secondary"
          sm
          bold
          onClick={() => navigate("/business-toolkit/pitch-deck-editor")}
        />
      </div>
      <div className="flex">
        <div className="flex flex-col gap-4 p-4">
          {slides.map((slide, i) => (
            <a href={`#${slide.id}`} key={slide.name}>
        ...
        <div ref={customPrintRef} >
          <Template1 data={data} slideContent={slideContent} />
        </div>
      </div>
    </div>
  );
