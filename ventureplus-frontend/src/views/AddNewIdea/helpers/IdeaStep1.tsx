...
const IdeaStep1 = ({ Form, form, questions, isFirstStep, valueData }: any) => {
  ...
  const [solvingAnswer, setSolvingAnswer] = useState<any>("");
  ...
  const handleAi = (
    ...
    if (aiDataBody) {
      ...
      fetch(
        ...
        // "https://vsrb4q5z-3000.inc1.devtunnels.ms/IdeaValidationAnswerSuggestion",
        {
          method: "POST",
          headers: {
        ...
        <Col lg={24} md={24} sm={24}>
          ...
          >
            ...
            {questions.map((question: any) => (
              <div key={question.id}>
                <h1 className="text-[#212838] md:text-lg text-md font-medium mb-3">
                  {question.question.replace("$", userCurrency ? userCurrency : "$")}
                </h1>
                {question.isMCQ ? (
                  <Form.Item
                  ...
                  >
                    <Radio.Group
                      className={`w-full green-radio-group gap-2 ${question?.mcqAnswers?.length > 2
                        ? ""
                    ...
                    >
                      {question.mcqAnswers.map((answer: any) => (
                        <Radio
                          key={answer.id}
                          value={answer.answer}
                          className="green-radio border border-[#D9D9D9] font-medium p-3 rounded-lg w-full text-[#4A5366] mb-2"
                        >
                          {answer.answer}
                        </Radio>
                      ))}
                  ...
                  >
                    <Input
                      ...
                      suffix={
                        <Button
                          type="primary"
                          className="!h-[32px] text-[#E3E7EF] w-[120px] !bg-[#7A5AF8] py-5 px-4 z-10"
                          onClick={() =>
                            handleAi(
                              question.id,
                              question.question.replace("$", userCurrency ? userCurrency : "$"),
