...
const OnBoardingBusinessType = ({
  ...
  const { Option } = Select;
  const [selectItem, setSelectItem] = useState<string | null>(value[item.keyValue]);
  ...
  const changeHandler = (val: any) => {
    ...
    setAnswer(val);
    setValue((prev: any) => ({ ...prev, [item?.keyValue]: val }));
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="rounded-lg">
        <label className="text-[15px] font-medium">Select Industry</label>

        <div className="flex mt-[10px] h-[40px] gap-3">
          {!custom ? (
            <Select
              className={"flex-1 h-full onBoardingSelectClassForbg"}
            ...
            <Input
              className="flex-1 h-full bg-[#FFFFFF]"
              value={answer}
              placeholder="Enter Business Industry"
