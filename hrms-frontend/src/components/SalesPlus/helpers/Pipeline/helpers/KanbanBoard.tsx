...
import {
  editFinalLeadApi,
  getAllPipelineStagesApi,
  getfinalLeadUsersApi,
...
const KanbanBoard = () => {
  ...
  const getAllPipelineStages = useSelector(
    (state: any) => state?.getAllPipelineStages
  );

  const addPipeLineStage = useSelector(
    (state: any) => state?.createPipelineStages
  );
  ...
  useEffect(() => {
    setTasks([]);
    setTasksTwo([]);
    getAllPipelineStagesApi(dispatch, Number(params?.id), onSccess);
  }, [params.id, addPipeLineStage?.data, editPipelineStage]);
  function onSccess(res: any) {
    for (let i = 0; i < res.length; i++) {
      const element = res[i].id;
      if (element) {
        getfinalLeadUsersApi(dispatch, Number(element), onSccessByPipeline);
      }
    }
  }
  ...
  return (
    <>
      {modalOpen && (
        <CreateStagModal open={modalOpen} close={() => setModalOpen(false)} />
      )}
      <div className=" mb-4 ">
        <div className={`bg-[#FFFFFF] dark:bg-dark-primary dark:text-white p-[15px] rounded-xl`}>
          <div>
            <div className="text-[18px] font-medium">
              <div className="text-[0.975rem] flex  items-center font-medium text-[] cursor-pointer ">
                <span onClick={() => navigate(-1)}>
                  <IoIosArrowBack className="px-[1px] text-[1rem]" size={25} />
                </span>
                <span onClick={() => navigate(-1)}>Back to Sales</span>
              </div>
            </div>
            <p className="text-[#1D2939] dark:text-white text-[0.813rem] font-medium">
              <span className="text-[#667085] dark:text-white" onClick={() => navigate(-1)}>
                Sales Plus{" "}
              </span>{" "}
          ...
          <div className={`flex justify-between items-center mt-[24px]`}>
            <div className="flex items-center gap-1">
              <div>
                <h2 className="text-[#344054] dark:text-white text-[1.44rem] font-semibold">
                  {location?.state?.title}
                </h2>
              </div>
            </div>
            <div className="flex gap-2">
              {user.role !== "agentStaff" && (
                <RoundedButton
                  className="dark:text-white dark:bg-dark-grayLight"
          ...
          <div>
            <div className="h-[720px] ">
              <div className="w-full flex  overflow-x-auto overflow-y-hidden items-center  mt-3 pb-[20px] custom-scrollbar">
                <DndContext
                ...
                >
                  {getAllPipelineStages?.data?.length > 0 ? (
                    <div className="flex gap-4 ">
                      <SortableContext items={columnsId} >
                        {getAllPipelineStages?.data?.map((col: any) => (
                          <ColumnContaier
                            key={col.id}
                            column={col}
                            ...
                            tasks={tasks.filter(
                              (task: any) => task?.pipelineStagesId === col?.id
                            )}
    ...
    if (isActiveATask && isOverAColumn) {
      setTasks((tasks) => {
        ...
        editFinalLeadApi(dispatch, {
          pipelineStagesId: event?.over?.id,
        ...
        copy.pipelineStagesId = event?.over?.id;
