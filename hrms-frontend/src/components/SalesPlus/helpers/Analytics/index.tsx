...
import {
  getLeadsSourceCountApi,
  getLeadsStatusCountApi,
...
const Analytics = (props: Props) => {
  ...
  const getLeadsSourceCount = useSelector(
    (state: RootState) => state.getLeadsSourceCount
  );
  const getLeadsStatusCount = useSelector(
    (state: RootState) => state.getLeadsStatusCount
  );

  useEffect(() => {
    if (getLeadsSourceCount?.data) {
      const data = getLeadsSourceCount?.data?.data;
  ...
  useEffect(() => {
    if (getLeadsStatusCount?.data) {
      const data = getLeadsStatusCount?.data?.data;
      setTotalLeadsByStatusValue(
        getLeadsStatusCount?.data?.totalLeadByStatusCount
      );
  ...
  useEffect(() => {
    getLeadsSourceCountApi(dispatch);
    getLeadsStatusCountApi(dispatch);
  }, []);
