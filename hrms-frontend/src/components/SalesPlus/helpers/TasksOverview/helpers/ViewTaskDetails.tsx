...
import { getTaskIdApi, markAsCompleteApi } from "../../../../../redux/api/SalesPlus/TasksOverview";

const ViewTaskDetails = ({ open, close, task, enumValue }: any) => {
  const viewTaskDetails = useSelector((state: any) => state.getTaskId);
  const dispatch = useDispatch();
  useEffect(() => {
    if (task?.id) {
      getTaskIdApi(dispatch, task?.id);
    }
  }, [task?.id]);
            ...
            <div className="py-[18px] px-[16px]  ">
              ...
              {viewTaskDetails?.data?.lead_id && (
                <div className="border-[1px] border-[#F2F4F7] rounded-[5px] h-[86px] mb-[18px]">
                  ...
                  <div
                    className="flex px-[12px] mt-[6px] items-center gap-2 cursor-pointer"
                    onClick={() => {
                      navigate(
                        `/sales-plus/${viewTaskDetails?.data?.lead_id}`
                      );
                    }}
                  >
                    <div className="w-[32px] h-[32px] bg-[#34405414] rounded-full text-[0.813rem] font-normal flex items-center justify-center">
