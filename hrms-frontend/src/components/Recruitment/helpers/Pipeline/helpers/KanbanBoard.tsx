...
import {
  getAllCandidatesByJobOpeningIdApi,
  openingStagesByJobIdApi,
  updateJobOpeningStageApi,
} from "../../../../../redux/api/Recruitment/pipeline";
...
const KanbanBoard = () => {
  ...
  const openingStagesByJobId = useSelector(
    (state: any) => state?.openingStagesByJobId
  );
  ...
  const updateJobOpeningStage = useSelector(
    (state: any) => state?.updateJobOpeningStage
  );
  ...
  useEffect(() => {
    setTasks([]);
    setTasksTwo([]);
    openingStagesByJobIdApi(dispatch, Number(params?.id), onSccess);
  }, [params?.id, addOpeningStage?.data]);
  function onSccess(res: any) {
    for (let i = 0; i < res?.data?.length; i++) {
      const element = res?.data?.[i]?.id;
      if (element) {
        getAllCandidatesByJobOpeningIdApi(
          dispatch,
          Number(element),
  ...
  useEffect(() => {
    if (updateJobOpeningStage?.data || socket || leadId) {
  ...
  }, [socket, updateJobOpeningStage?.data, leadId]);
        ...
        <div className={`bg-[#FFFFFF] p-[15px] rounded-xl`}>
          <div>
            <div className="text-[18px] font-medium">
              <div className="text-[0.975rem] flex  items-center font-medium text-[rgb(95,116,184)] cursor-pointer ">
          ...
          <div className={`flex justify-between items-center mt-[24px]`}>
            ...
            <div className="flex gap-2">
              {user.role !== "agentStaff" && (
                ...
                >
                  <span className="text-[rgb(95,116,184)] text-[0.813rem] font-medium flex items-center gap-1">
          ...
          <div>
            <div className="h-[720px] ">
              <div className="w-full flex  overflow-x-auto overflow-y-hidden items-center  mt-3 pb-[20px]">
                ...
                >
                  {openingStagesByJobId?.data?.data?.length > 0 ? (
                    <div className="flex gap-4">
                      <SortableContext items={columnsId}>
                        {openingStagesByJobId?.data?.data?.map((col: any) => (
                          <ColumnContaier
                            key={col.id}
                            column={col}
                            ...
                            tasks={tasks.filter(
                              (task: any) => task?.openStageId === col?.id
                            )}
  ...
  function onDragover(event: DragOverEvent) {
    ...
    if (isActiveATask && isOverAColumn) {
      setTasks((tasks) => {
        ...
        updateJobOpeningStageApi(dispatch, {
          stageId: event?.over?.id,
          candidateId: event.active.data.current
            ? event.active.data.current?.task?.id
            : 0,
        });
        if (event.active.data.current) {
          setLeadId(event.active.data.current.task.id);
        }
        copy.openStageId = event?.over?.id;
