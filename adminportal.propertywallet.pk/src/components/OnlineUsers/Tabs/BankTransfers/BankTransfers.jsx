...
import bankTransferColumns from '../../../../tableColumns/bankTransferColumns.json'
...
import {
  getAllBankRequestsApi,
  getAllOnlineUsers,
} from '../../../../redux/api/OnlineUsers'
import moment from 'moment'
import { updateWithdrawRequestsBankRequestApi } from '../../../../redux/api/WithdrawRequests'
import BankSlip from '../../Print/BankSlip'

import BTFilter from './BTFilter'
...
const BankTransfers = () => {
  ...
  const getAllUsers = useSelector((state) => state?.getAllBankRequest)
  const { data, loading } = useSelector(
    (state) => state?.updateWithdrawRequests
  )
  const debouncedgetAllBankRequests = useRef(
    debounce((dispatch, pageLimit, data, search, selectedFilter) => {
      getAllBankRequestsApi(dispatch, pageLimit, data, search, selectedFilter)
    }, 500)
  ).current
  useEffect(() => {
    scrollToTop()
    debouncedgetAllBankRequests(dispatch, pageLimit, search, selectedFilter)
  }, [pageLimit, data])
  const handleRequest = (req, id) => {
    updateWithdrawRequestsBankRequestApi(
      dispatch,
      { status: req },
      id,
      onSuccess
    ...
    if (getAllUsers?.data?.data !== null) {
      const data = getAllUsers?.data?.data?.items.map((item, i) => {
        return {
          ...
          voucherNo: (
            <span className="font-medium text-[12px] leading-[18px] text-[#3D4350]">
              {item?.bankRecieptNo !== null && item?.bankRecieptNo !== ''
                ? item?.bankRecieptNo
                : '-'}
            </span>
          ),
