...
const AddAreaDetails = ({ current }) => {
  ...
  const projectId =
    updateProjectInventoryStepOne?.data?.data?.projectTypeId ||
    createProjectInventoryStepOne?.data?.data?.projectTypeId
  const noOfUnit =
  ...
  useEffect(() => {
    if (current === 3) {
      getAllPlotsApi(dispatch, params?.id)
    }
  ...
  return (
    <>
      <AddPlotDetail visible={visible} toggle={toggle} />
      <SectionContainer
