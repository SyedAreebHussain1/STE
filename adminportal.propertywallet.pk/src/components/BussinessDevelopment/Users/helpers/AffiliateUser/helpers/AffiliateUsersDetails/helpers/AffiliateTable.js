...
const AffiliateTable = ({
  ...
  useEffect(() => {
    if (AffiliateDetails?.data) {
      const data = AffiliateDetails?.data?.data?.items?.map((item) => {
        return {
      ...
      <Row className="bg-white">
        <Col lg={24} xs={24}>
          ...
          <Table
            ...
            scroll={{ x: true }}
            loading={AffiliateDetails?.loading}
            pagination={{
              total:
                AffiliateDetails?.data?.data?.meta?.totalPages *
                AffiliateDetails?.data?.data?.meta?.itemsPerPage,
