...
import { GetELWithdrawListApi } from '../../../../redux/api/ELoungeWithdraw'
...
const WithdrawRequestTable = ({ setExcelSheetData }) => {
  ...
  const { data: WithdrawList, loading: WithdrawLoading } = useSelector(
    (state) => state.getELWithdrawList
  )
  const { data: Withdraw } = useSelector((state) => state.postELWithdrawAmount)
  ...
  useEffect(() => {
    scrollToTop()
    GetELWithdrawListApi(dispatch, pageLimit)
  }, [pageLimit, Withdraw])
  return (
    <>
