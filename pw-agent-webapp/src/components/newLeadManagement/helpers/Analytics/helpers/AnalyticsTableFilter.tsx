...
const AnalyticsTableFilter = ({ pageLimit, setPageLimit }: any) => {
  ...
  useEffect(() => {
    if (userRole !== "agentStaff") {
      getStaffApi(dispatch, loadMorePageLimit, onSuccess);
    }
  }, []);
  ...
  const onLoadMore = () => {
    getStaffApi(
      dispatch,
      { page: loadMorePageLimit.page + 1, limit: loadMorePageLimit.limit },
      onLoadSuccess
    );
  ...
  return (
    <div className="flex justify-between">
        ...
        <Row gutter={10}>
          <Col className="mt-[10px]">
            <Input
              ...
              value={filter.search}
              onChange={(e) => {
                setFilter((pre: any) => ({ ...pre, search: e.target.value }));
              ...
              <DatePicker
                ...
                onChange={(event: any) => {
                  if (event) {
                    setFilter((prev: any) => ({
                      ...prev,
                      endDate: dayjs(event)?.hour(23)?.minute(49)?.second(49),
                    }));
                  } else {
                    setFilter((prev: any) => ({ ...prev, endDate: null }));
                  }
                }}
      ...
      <div className="flex items-center h-[40px] mt-[10px]">
        <button
          className="text-[red] disabled:text-[gray] disabled:cursor-not-allowed font-semibold mr-[10px]"
          disabled={!isReset}
          onClick={() => {
            setFilter({
              staffName: null,
