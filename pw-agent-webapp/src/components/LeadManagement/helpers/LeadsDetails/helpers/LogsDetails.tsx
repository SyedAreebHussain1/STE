...
import { getLeadlogApi } from "../../../../../redux/api/LeadManagement";
...
import { WhatsAppOutlined, StarOutlined, PhoneOutlined, MailOutlined, BookOutlined } from "@ant-design/icons";
  ...
  return (
    <>
      ...
      <div className="mt-4 bg-[#FFFFFF] p-[15px] rounded-xl ">
        <h4 className="text-[#344054] text-[1.1875rem] font-medium mb-2">
          Logs
        </h4>
                ...
                return (
                  <div key={i} className="mt-10">
                    ...
                    <div className="flex items-center gap-1">
                      {" "}
                   {
                    item?.logStatus === "whatsapp" ? <WhatsAppOutlined/> : item?.logStatus === "statusChange" ?  <StarOutlined/> : item?.logStatus === "appointment" ? <BookOutlined />: item?.logStatus === "email" ? <MailOutlined />: <PhoneOutlined className="scale-x-[-1]" /> 
                   }
                      {/* {" "} */}
                      <span className="text-[1rem] font-semibold text-[#292D35] ">
                        {" "}
                      ...
                      {!item?.leadlogNote?.note && (
                        <button
                          className="text-[#3D4350] text-[.75rem] font-semibold flex items-center  h-[24px] rounded-full border p-1 "
                          onClick={() => setToggleId(item?.id)}
                        >
                          {" "}
                          <img src={plusIcon} alt="" className="object-cover mr-1" /> Add note
                        </button>
                      )}
                    </div>
                    <p className="text-[#667085] text-[.75rem] font-medium">
                      {moment(item.createdAt).format("YYYY-MM-DD hh:mm A")}
                    </p>
                    <p className="text-[#667085] text-[.75rem] font-medium">
                    Created By: {item?.createdByUser?.profile?.fullName}
                    </p>
                        ...
                        {item?.leadlogNote?.note && (
                          <div className="flex mt-6 w-full ">
                            ...
                            <div className="w-full ml-1">
                              <p className="text-[#667085] text-[.875rem] font-medium ml-1">
                                Note Added
                              </p>
                              <div className="p-[10px] border w-full rounded-[8px] inline-block ml-1">
                                <p className="text-[#667085] text-[.75rem] font-bold">
                                ...
                                <textarea
                                  className="text-[#667085] text-[.75rem] font-medium bg-transparent  resize-none w-full mt-1"
                                  disabled
