...
const BlogsUpdateModal = ({ open, close, type }: Props) => {
  const [form] = useForm();
  const [
    images,
    setImages,
  ...
  function onFinish(values: {
    ...
    updateAllBlogsApi(dispatch, formData, Number(open.id), onSuccess);;
  }
  ...
  useEffect(() => {
    if (type) {
      form.setFieldsValue({
        blogsCategoryId: open?.blogCategory?.title,
        title: open.title,
        blog: open?.blog,
        author: open?.author,
        type: open?.type,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex gap-2 justify-between items-center w-full">
          <div className="w-[100%]">
            <label htmlFor="blog" className="text-md font-semibold">
              Enter Blog
            </label>
            <TextInput
              id="blog"
...
export default BlogsUpdateModal;
