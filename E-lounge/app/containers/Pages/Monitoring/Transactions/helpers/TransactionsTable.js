...
import { getAllTransactionHistoryApi } from "../../../../../redux/modules/Monitoring/action";
// import allSignUpsColumn from "../../../../../utils/columns/allSignUpsColumn";
import allTransactionsColumns from "../../../../../utils/columns/allTransactionsColumns";
...
const TransactionsTable = () => {
  ...
  const getAllTransaction = useSelector((state) =>
    state.getIn(["getAllTransactionHistory"])
  );
  ...
  useEffect(() => {
    getAllTransactionHistoryApi(dispatch, pageLimit);
  }, [pageLimit]);
    ...
    if (getAllTransaction?.data) {
      if (getAllTransaction?.data?.data?.items?.length > 0) {
        const data = getAllTransaction?.data?.data?.items?.map((item) => {
          ...
          return [
            item?.eLoungeSaleUserWallet?.eloungeSaleUser?.eloungeUser
              ?.fullName || "-",
            item?.sourceBy,
            item?.paymentFor,
            item?.amount,
        ...
        <Paper>
          <TableContainer>
            <Table className={classes.table}>
              <TableHead>
                <TableRow>
                  {allTransactionsColumns.map((column) => (
