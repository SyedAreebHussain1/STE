...
import { getSignUpCountsApi } from "../../../../redux/modules/Tracking/action";
import { getFromStorage } from "../../../../utils/storage";
import ViewLocationForSignUpsModal from "../../Tracking/SignUps/helpers/ViewLocationForSignUpsModal";
import { getAssignedAgenciesManager } from "../../../../redux/modules/AssignedAgencies/action";
...
const AssignedAgenciesTable = ({ fullName, dateValue, renderAgain, setRenderAgain }) => {
  ...
  const getAllAssignedAgenciesManager = useSelector((state) => state.getIn(["getAllAssignedAgenciesManager"]));
  ...
  useEffect(() => {
    if (getAllAssignedAgenciesManager?.data) {
      if (getAllAssignedAgenciesManager?.data?.data?.items?.length > 0) {
        const data = getAllAssignedAgenciesManager?.data?.data?.items?.map((item, i) => {
          return [
            i + 1,
  ...
  }, [getAllAssignedAgenciesManager.data]);
  return (
    <>
      {editModal.visible && (
        ...
        <Grid item xs={12} md={12}>
          <Paper>
            ...
            <TablePagination
              ...
              component="div"
              count={
                getAllAssignedAgenciesManager.data ? getAllAssignedAgenciesManager.data.data.meta.totalItems : 0
              }
